resource "azurerm_network_security_rule" "inbound_security_rules" {
  ...
  protocol                                   = each.value.protocol != null ? each.value.protocol : "Tcp" # default protocol to TCP
  source_port_range                          = "*"
  ...
  access                                     = each.value.action != null ? each.value.action : "Allow" # default to Allow if not specified
