variable "cidr_block" {
  type = object({
    ...
    ipv4 = object({
      ...
      ///
      /// @link {vpc-ipam} https://docs.aws.amazon.com/vpc/latest/ipam/what-it-is-ipam.html
      /// @since 1.0.0
...
variable "enable_network_address_usage_metrics" {
  ...
  description = <<EOT
    ...
    @link {vpc-network-address-usage} https://docs.aws.amazon.com/vpc/latest/userguide/network-address-usage.html
    @since 1.0.0
  EOT
  default     = false
}
...
variable "peering_connection_requests" {
  ...
  description = <<EOT
    ...
    @example "Peering Connection Requests" #peering-connection-requests
    @since 1.0.0
  EOT
  default     = {}
}
