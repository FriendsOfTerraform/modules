locals {
  route_table_propagations = flatten([
    for k, v in var.route_tables : [
      for propagation in v.propagations :
      {
        route_table_name = k
...
resource "aws_ec2_transit_gateway_route_table_propagation" "route_table_propagations" {
  for_each = toset([for k in local.route_table_propagations : "${k.route_table_name}~${k.attachment_name}"])
